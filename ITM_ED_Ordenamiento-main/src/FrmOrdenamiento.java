...
public class FrmOrdenamiento extends JFrame {
    ...
    private void btnOrdenarInsercionClick(ActionEvent evt) {
        if (cmbCriterio.getSelectedIndex() >= 0) {
            Util.iniciarCronometro();
            Documento.ordenarInsercion(cmbCriterio.getSelectedIndex()); // Llama al método de inserción
            txtTiempo.setText(Util.getTextoTiempoCronometro());
            Documento.mostrarDatos(tblDocumentos); // Actualiza la tabla con los datos ordenados
        }
    }
