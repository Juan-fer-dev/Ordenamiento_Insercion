...
public class Documento {
    ...
    public static void ordenarInsercion(int criterio) {
        for (int i = 1; i < documentos.size(); i++) {
            ...
            // Comparamos el documento actual con los anteriores según el criterio
            while (j >= 0 && esMayor(documentos.get(j), actual, criterio)) {
                documentos.set(j + 1, documentos.get(j)); // Mueve los elementos mayores hacia la derecha
                j--;
            }
    
            documentos.set(j + 1, actual); // Inserta el documento en su lugar correcto
        }
    }
